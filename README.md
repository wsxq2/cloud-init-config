...
使用时先根据实际情况修改一下各个配置文件中的内容，然后直接复制到烧录后的 TF 卡中的 boot 分区根目录即可，该目录下已经有相关文件，我们需要覆盖它。boot 分区是 FAT 分区，将 TF 卡通过读卡器插上后电脑可以看到此分区，所以可以很方便地复制文件。
...
此外，首次开机可能无法连接到 `network-config` 中设置的 WIFI，重启一下就好了。
